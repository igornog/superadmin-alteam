...
import { Currency, ListingStatus } from '@yjcapp/app'
...
export const sortBy = (sort: string, items: any[]) => {
  let arrayForSort = []
  let listSorted = []
  ...
  switch (sort) {
    ...
    case ListingStatus.Active:
      arrayForSort = [...items]
      listSorted = arrayForSort.sort((a, b) =>
        a.status === ListingStatus.Active ? -1 : 1,
      )
      break
    case ListingStatus.Disabled:
      arrayForSort = [...items]
      listSorted = arrayForSort.sort((a, b) =>
        a.status === ListingStatus.Disabled ? -1 : 1,
      )
      break
    case ListingStatus.Ended:
      arrayForSort = [...items]
      listSorted = arrayForSort.sort((a, b) =>
        a.status === ListingStatus.Ended ? -1 : 1,
      )
      break
  }
