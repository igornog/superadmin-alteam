...
export function createFreelancer(freelancer: any) {
  ...
  if (freelancer.email !== undefined) {
    return freelancerPgRepository.createGroup(transformGroupWebFlowRequest(freelancer))
  }
  return freelancerPgRepository.createFreelancer(transformSoloWebFlowRequest(freelancer))
}
