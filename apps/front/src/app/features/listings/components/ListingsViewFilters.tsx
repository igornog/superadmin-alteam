...
const ListingsViewFilters: React.FunctionComponent = () => {
  const settings = useAppSelector((state) => state.settings)
  const dispatch = useAppDispatch()
  const app = useAppSelector((state) => state.app)

  const handleClick = (filter: Filter, section: 'clients' | 'listingStatus' | 'priceRange') => {
    ...
    <Grid item={true} style={{ width: '235px' }}>
      <StyledFilters isFixed={app.sidePanel.isFixed}>
        <Box display={'flex'} flexDirection={'column'}>
          ...
          <AtTypography variant={'h5'}>Client</AtTypography>
          ...
          >
            ...
            {settings.filters.clients?.map((filter: Filter, index: number) => (
              <StyledBox
                display={'flex'}
                width={'100%'}
                ...
                onClick={() => handleClick(filter, 'clients')}
              >
          ...
          <AtTypography variant={'h5'}>Status</AtTypography>
          ...
          <Box
            ...
            gap={1.5}
          >
            {settings.filters.listingStatus?.map(
              (filter: Filter, index: number) => (
                <StyledBox
                  ...
                  key={index}
                  $active={filter.active}
                  color={filter.active ? black : grey2}
                  onClick={() => handleClick(filter, 'listingStatus')}
                >
                  <AtTypography>{filter.label}</AtTypography>
                  <StyledAddCircle size={20} $active={filter.active} />
                </StyledBox>
          ...
          <AtTypography variant={'h5'}>Price Range</AtTypography>
          ...
          <Box
            ...
            gap={1.5}
          >
            {settings.filters.priceRange?.map(
              (filter: Filter, index: number) => (
                <StyledBox
                  display={'flex'}
                  width={'100%'}
                  ...
                  $active={filter.active}
                  color={filter.active ? black : grey2}
                  onClick={() => handleClick(filter, 'priceRange')}
                >
                  <AtTypography>{filter.label}</AtTypography>
                  <StyledAddCircle size={20} $active={filter.active} />
                </StyledBox>
...
export default ListingsViewFilters
