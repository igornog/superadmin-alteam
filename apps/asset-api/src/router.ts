...
export const router = express.Router().use(fileUpload())
...
const assetService: AssetService = {
  async createTalentAsset(
    talentId: string,
    soloTalentAsset: SoloTalentAsset,
  ): Promise<string> {
    const assetId = v4()
    const path = `users/${talentId}/${assetId}`
    await saveAsset(
      path,
      Buffer.from(await soloTalentAsset.asset.arrayBuffer()),
    )
    ...
    await soloTalentPgRepository.updateSoloTalent({
      ...talent,
      assets: talentAssets,
    })
...
async function uploadSoloTalentAsset(
  talentId: string,
  file: Buffer,
): Promise<string> {
  const assetId = v4()
